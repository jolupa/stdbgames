...
<?php else: ?>
  ...
  <?php else: ?>
    ...
    | Pri <?php if ( $game['price'] == '' ): ?>TBA<?php else: ?><?= $game['price'] ?> €<?php endif; ?><?php if ( $game['pro'] == 1 && $game['pro_from'] <= date ('Y-m-d') ): ?> | Free for Pro<?php elseif ( $game['pro'] == 1 && $game['pro_from'] > date ('Y-m-d') ): ?>Will Be Free for Pro on Launch<?php endif; ?>
