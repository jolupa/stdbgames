...
<?php elseif ( isset ( $price ) ): ?>
  Pri <?php if ( $price['price'] == '' ): ?>TBA<?php else: ?><?= $price['price'] ?> €<?php endif; ?><?php if ( $price['pro'] == 1 && $price['pro_from'] <= date ('Y-m-d') ): ?> | Free for Pro<?php elseif ( $price['pro'] == 1 && $price['pro_from'] > date ('Y-m-d') ): ?> | Free on Launch<?php endif; ?>
<?php endif; ?>
