			...
			<table class="is-fullwitdth is-hoverable">
				...
				<?php foreach($gamehasaddons as $gamehasaddons): ?>
					<tr>
						...
						<td><?php if(session('role') == 1): ?><a title="Delete Addon" href="<?= base_url() ?>/addons/deleteaddon/<?= number_format($gamehasaddons['aId'], 2) ?>"><span class="icon"><i class="fas fa-trash-alt"></i></span></a>&nbsp;<a title="Edit Addon" href="<?= base_url() ?>/addons/update/<?= $gamehasaddons['aSlug'] ?>"><span class="icon"><i class="fas fa-edit"></i></span></a><?php endif; ?></td>
					</tr>
